...
```
(d) The lowest p.value obtained before matching is 0.106 and the lowest p-value obtained after matching is 0.41475. The two p values are not statistically significant, therefore it performs well and meets our balance standard. Our balance standard is typically met when there is no statistical significance (i.e. p value > 0.05) in covariates between the treated and control groups after matching and these p-values did a great job at that. The high p-values indicate that there is balance between the treated and control group, which is what we want to make causal inferences. This results are way better than the propensity score matching p-values which are statistically significant.   

(e) The treatment effect of 0.045073 suggests that, on average, individuals exposed to violence are more likely to seek peace holding other factors constant. Our standard error of 0.023319  indicates greater confidence in the estimated treatment effect. However, the p-value of 0.05325, slight larger than the standard of 0.05, shows that the estimated effect is not statistically significant at the traditional level. This suggests that there may be a relationship between direct exposure to violence and seeking peace or vengeance, but we will need more information to confirm it with greater certainty. 
...
In summary, genetic matching has shown to be a better matching tool compared to propensity score matching. In the case of propensity score matching, our before and after matching results provided a very high statistical significance, which is contrary to the effect we wanted to see for indicating balance between the treated and control groups. However, after using genetic matching, our p values significantly improved and were above 0.05, indicating there is no statistical significance and there is a balance between the treated and control groups. This could be because genetic matching reliably reduces both the bias and the mean square error of the estimated causal effect even when the property of equal percent bias reduction (EPBR) does not hold (Diamond et al., 2005).
Also, our treatment effect indicated that, on average, individuals exposed to violence are more likely to seek peace holding other factors constant, and this was supported by our low standard error of 0.0089193 and p-value < 2.22e-16, showing a statistical significant result. The final evidence comes from the unadjusted estimate in our sensitivity analysis results suggesting a positive relationship between direct exposure to violence and seeking peace, which is shown by the positive coefficient estimate of 0.0973. However, the partial R2 values indicate that extreme unobserved confounders would need to account for a minimum of 2.19% of the residual variance of the treatment to entirely justify the observed estimated effect, indicating a moderate sensitivity to unobserved confounding. Moreover, the robustness values serve as benchmarks for evaluating the impact of unobserved confounders. In our scenario, if unobserved confounders explain more than 13.88%% (or 7.63% at alpha = 0.05) of the residual variance of both the treatment and the outcome, they could significantly skew the estimated effect. Therefore, despite the apparent positive association between direct exposure to violence and seeking peace, the sensitivity analysis underscores the importance of accounting for potential unobserved confounders.
...
The synthetic control analysis for Cantabria suggests a notable impact on its per-capita GDP. Before the treatment period of the terrorism in the Basque country(pre-1969), Cantabria's GDP trend was comparable to the synthetic Cantabria. However, following the treatment period (post-1969), a minor gap emerges, indicating a potential negative effect on Cantabria's GDP. This suggests that the events of the terrorism in the Basque country had a negative effect in Cantabria post-1969, which have led to a decline in its economic performance relative to the synthetic control. A possible explanation could be the withdrawal of foreign investment from this region because of the fear of the possible impacts the terrorism activities in the Basque country could have in Cantabria. Nevertheless, further analysis and interpretation are warranted to fully understand the implications of these findings.
